...
1. **Be explicit and verified.** If you don't know, say "I don't know — I need more context." Confirm behavior against source or current docs before acting.
2. **Be surgical.** Touch only what the mission requires. Match existing style. Leave unrelated code alone.
3. **Capture durable findings.** If it took investigation or would be costly to rediscover, add a note to `.notebook/`.
4. **Use pointers, not copies.** Reference code by `file:function()` or `file` (L10-25). Never paste code blocks into notes.

## Mission Cycle

Use this cycle. Trivial tasks may compress it, but do not skip needed steps.
...
2. Read the developer's request. Identify:
...
4. Check what tools, skills, and MCPs are available in the current environment. Note them for later use.
...
Prioritize in this order: correctness, scope, style, then brevity.
